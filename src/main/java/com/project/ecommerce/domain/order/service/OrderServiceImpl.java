...
import com.project.ecommerce.common.exception.AddressErrorMessages;
import com.project.ecommerce.common.exception.AddressException;
           ...
           try {
               ...
               switch (request.getDeliveryOption()) {
                   case DEFAULT_ADDRESS -> {
                        // 사용자의 기본 배송지 조회
                       shippingAddress = addressRepository.findByUserAndIsDefault(user, true)
                   ...
                   case SAVED_ADDRESS -> {
                       ...
                       shippingAddress = addressRepository.findByIdAndUser(request.getAddressId(), user)
                   ...
                   default -> {
                       throw new AddressException(AddressErrorMessages.INVALID_DELIVERY_OPTION, HttpStatus.BAD_REQUEST);
                   }
               }
